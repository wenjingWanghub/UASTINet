...
def random_regular_mask(img):
    ...
    for _ in range(N_mask):
        ...
        range_x = x + random.randint(30, 80)
        range_y = y + random.randint(30, 80)
        mask[:, int(x):int(range_x), int(y):int(range_y)] = 1
    return mask
...
def random_irregular_mask(img):
    ...
    mask = torch.zeros_like(img)
    size = img.size()
    img = np.ones((size[1], size[2], 1), np.uint8)
    ...
    for _ in range(number):
        ...
        if model < 0.6:
            ...
            thickness = randint(4, max_width)
            cv2.ellipse(img, (x1, y1), (s1, s2), a1, a2, a3, (256, 256, 256), thickness)
